...
# Input: undersamplingFaktor (0 = kein Undersampling, 0.99 = Um 99 %, also 1% der urspr. Anzahl), Trainingsdatensatz
undersample_2 <- function (undersamplingFactor = 0.99, dataset = ccdata){
...
testedHyperparameters <- read.csv("hyperparameter_tuning_V2.csv")[,-1]
...
for (i in 1:500){
  ...
  write.csv(testedHyperparameters, "hyperparameter_tuning_V2.csv", row.names=TRUE)
}
