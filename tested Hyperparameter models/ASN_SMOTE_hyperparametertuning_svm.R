...
singleModelRun <- function (kSMOTE = 5, nSMOTE = 577, train, test){
  ...
  params <- makeParamSet(
    makeNumericParam("cost", lower = 0.1, upper = 10),  # hier vllt die spanne kleiner
    makeNumericParam("gamma", lower = 0.001, upper = 1), # hier vllt die spanne kleiner
  ...
  best_cost <- tune_result$x$cost
  best_gamma <- tune_result$x$gamma
  best_kernel <- tune_result$x$kernel
...
kFoldCrossValidate <- function(kForFold = 4, 
  ...
  return(list(
    ...
    "cost" = resultsOfSingleRun$cost,
    "gamma" = resultsOfSingleRun$gamma,
    "kernel" = resultsOfSingleRun$kernel 
  ))
}
...
tryRandomHyperparameter <- function (kForFold,
  ...
  resultVector <- c(kForFold, undersamplingFactorToTest, kToTest, nRatioToTest, crossValidatedAUC$AUCAverage, crossValidatedAUC$cost, crossValidatedAUC$gamma, crossValidatedAUC$kernel)
  return(resultVector)
}
...
col_names <- c("kFold", "undersamplingFactor", "kSMOTE", "nRatioSMOTE", "AUCAverage", "cost", "gamma", "kernel")
hyperparameter_svm_results <- data.frame(matrix(ncol= length(col_names),nrow=0))
colnames(hyperparameter_svm_results) <- col_names
...
for (i in 1:500){
  ...
  hyperparameter_svm_results <- rbind(hyperparameter_svm_results, newRandomHyperparameterTestResult)
  cat("---------------------------- Abgeschlossener Durchlauf: ", i, " ----------------------------")
  
  #write.csv(hyperparameter_svm_results, "testedHyperparameters.csv", row.names=TRUE)
}
