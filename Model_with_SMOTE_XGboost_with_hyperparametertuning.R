...
hyperparameterOptimizeWithRandomSeach <- function (kForFold = 5,
  ...
  for (i in 1:amountOfRandomSearches){
    ...
    testResults <- rbind(testResults, resultVector)
    cat("Aktueller Forschritt: ", i, " von ", amountOfRandomSearches)
  }
  
  colnames(testResults) <- c("kFold", "kSMOTE", "nRatioSMOTE", "AUC Average")
...
testHyperParameterOptimize1 <- hyperparameterOptimizeWithRandomSeach(
  kForFold = 5,
  entireDataset = ccdata,
  kForSMOTERange = (1:15),
  nDesiredRatioSMOTERange = c(0.8,1.2),
  amountOfRandomSearches = 10
)

testHyperParameterOptimize2 <- hyperparameterOptimizeWithRandomSeach(
  kForFold = 5,
  entireDataset = ccdata,
  kForSMOTERange = (1:15),
  nDesiredRatioSMOTERange = c(0.8,1.2),
  amountOfRandomSearches = 25
)

testHyperParameterOptimize3 <- hyperparameterOptimizeWithRandomSeach(
  kForFold = 5,
  entireDataset = ccdata,
  kForSMOTERange = (1:15),
  nDesiredRatioSMOTERange = c(0.8,1.2),
  amountOfRandomSearches = 50
)
...
#write.csv(df, "hyperparameterTuning.csv", row.names=TRUE)
...
>>>>>>> 2f04684f47d84acfc90bc004d727ae2d137b492c
